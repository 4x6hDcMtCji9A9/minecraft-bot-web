...
async function sendCustomCommand(){
    ...
    try{
        const r=await fetch('/api/bots/command-all',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command})});
        const d=await r.json();
        showNotification(`✅ "${command}" enviado para ${d.sentCount}/${d.totalBots} bots!`,'success');
        input.value='';
    }catch(e){showNotification('❌ Erro ao enviar comando','error');}
}
